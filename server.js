...
try {
  ...
  console.log("✅ Multer загружен - поддержка файлов включена")
} catch (error) {
  console.log("⚠️ Multer не найден - загрузка файлов отключена")
}
...
// Загрузка файлов
app.post("/api/upload", checkDomain, (req, res) => {
  if (!upload) {
    return res.status(503).json({
      error: "Загрузка файлов временно недоступна",
      message: "Multer не установлен",
    })
  }
